...
const categoryIT=({navigation})=>{
  ...
  const [category]=useState([
    {icon:'ios-desktop',title:'Information Technology',key:'1',color:'green'},
    {icon:'ios-briefcase',title:'Business Management',key:'2',color:'orange'},
    ...
    {icon:'ios-school',title:'Robotics Engineering',key:'13',color:'#007FFF'},
  ]);

  return(
    <View style={{flex:1}}>
      <View style={{paddingVertical:20,backgroundColor:colors.defaultRed}}>
      <TextInput style={styles.searchBox} placeholder='Search By Category' placeholderTextColor='#666'></TextInput>
      ...
      renderItem={({item})=>{
        return(
            <TouchableOpacity
            onPress={()=>navigation.navigate('CategoryIT')}
            >
              <View style={styles.cateContainer}>
                <View styles={{width:'20%'}}>
                <Ionicons name={item.icon} size={30} color={item.color} style={{marginLeft:35}} />
                </View>
                <View style={{alignSelf:'center',paddingLeft:40,width:'80%'}}>
